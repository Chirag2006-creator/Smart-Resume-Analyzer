...
import { Upload, FileText, Briefcase, CheckCircle, AlertCircle, Loader2, ChevronRight, BarChart, Sparkles, TrendingUp, TrendingDown, Star, Lightbulb, User, Link as LinkIcon } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-6">
              ...
              <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-indigo-500/30 rounded-2xl cursor-pointer bg-indigo-500/5 hover:bg-indigo-500/10 hover:border-indigo-400 transition-all group">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="mb-2 text-sm text-slate-300">
                    <span className="font-semibold text-indigo-300">Click to upload</span> or drag & drop
                  </p>
            ...
            >
              {loading ? (
                <>
                  <Loader2 className="w-6 h-6 mr-3 animate-spin" /> Analyzing...
                </>
              ) : (
                <>
                  Launch Analysis <ChevronRight className="w-5 h-5 ml-1" />
                </>
              )}
            </button>
            ...
            {result ? (
              <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 backdrop-blur-xl border border-indigo-500/30 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        ...
                        <div className="flex items-center justify-center w-20 h-20 rounded-full border-4 border-indigo-500/30 bg-black/40 shadow-inner relative">
                          <span className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-br from-indigo-300 to-purple-300">
                            {result.match_score}%
                          </span>
                          <svg className="absolute inset-0 w-full h-full -rotate-90">
                            ...
                            <circle 
                              cx="40" cy="40" r="36" 
                              className="text-indigo-400 stroke-current transition-all duration-1000 ease-out" 
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {llm.experience_level && (
                            <span className={`text-xs px-2.5 py-1 rounded-full border font-semibold ${expLevelColor[llm.experience_level] || 'text-slate-300 bg-white/5 border-white/10'}`}>
                              {llm.experience_level}
                            </span>
                          )}
                        </div>
                  ...
                  {llm?.error && (
                    <div className="bg-white/5 backdrop-blur-xl border border-amber-500/20 rounded-3xl p-6 shadow-2xl">
                      <h3 className="text-lg font-bold text-amber-300 flex items-center gap-2 mb-2">
                        <AlertCircle className="w-4 h-4" /> LLM Analysis Unavailable
                      </h3>
                ...
                {hasLlm && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {llm.weaknesses && llm.weaknesses.length > 0 && (
                      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
                      ...
                      <div className="bg-gradient-to-r from-amber-900/20 to-orange-900/10 backdrop-blur-xl border border-amber-500/20 rounded-3xl p-6 shadow-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {llm.recommendations.map((r, i) => (
                            <div key={i} className="p-4 bg-black/20 rounded-xl border border-amber-500/10">
                              <span className="text-xs text-amber-500 font-bold mb-1 block">#{i + 1}</span>
                    ...
                    {llm.fit_analysis && (
                      <div className="bg-gradient-to-r from-cyan-900/20 to-blue-900/10 backdrop-blur-xl border border-cyan-500/20 rounded-3xl p-6 shadow-2xl">
                        <h3 className="text-lg font-semibold text-cyan-300 mb-3 flex items-center gap-2">
                          <User className="w-5 h-5" /> Job Fit Analysis
                        </h3>
                  ...
                  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
                    ...
                    {result.entities.skills.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {result.entities.skills.map((skill, idx) => (
                          <span key={idx} className="px-3 py-1.5 bg-emerald-500/10 text-emerald-300 text-xs font-medium rounded-lg border border-emerald-500/20">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.missing_skills.map((skill, idx) => (
                          <span key={idx} className="px-3 py-1.5 bg-pink-500/10 text-pink-300 text-xs font-medium rounded-lg border border-pink-500/20">
                            {skill}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      {result.entities.emails.length > 0 ? (
                        <ul className="space-y-1">
                          {result.entities.emails.map((e, i) => <li key={i} className="text-sm text-slate-300">{e}</li>)}
                        </ul>
                    ...
                    <div>
                      ...
                      {result.entities.phones.length > 0 ? (
                        <ul className="space-y-1">
                          {result.entities.phones.map((p, i) => <li key={i} className="text-sm text-slate-300">{p}</li>)}
                        </ul>
                      ) : <span className="text-sm text-slate-600">—</span>}
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider mb-2 font-semibold flex items-center gap-1">
                        <LinkIcon className="w-3 h-3"/> Links
                      </p>
                      {result.entities.links && result.entities.links.length > 0 ? (
                        <ul className="space-y-1">
                          {result.entities.links.map((l, i) => (
                            <li key={i}>
                              <a href={l} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-300 hover:text-indigo-200 underline truncate block">{l}</a>
                            </li>
                          ))}
                        </ul>
                      ) : <span className="text-sm text-slate-600">—</span>}
                      ...
                      {result.entities.organizations.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {result.entities.organizations.map((org, i) => (
                            <span key={i} className="px-2 py-1 bg-white/5 text-slate-300 text-xs rounded-md border border-white/10">{org}</span>
                          ))}
                        </div>
                      ) : <span className="text-sm text-slate-600">—</span>}
                    </div>
              ...
              <div className="h-full min-h-[500px] bg-white/[0.02] border border-white/[0.05] border-dashed rounded-3xl flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-slate-500 max-w-sm mb-6">
                  Upload a resume and optional job description to see the hybrid AI analysis results.
                </p>
